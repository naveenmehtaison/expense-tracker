...
import { IncomeReducerAction } from "../Store/IncomeReducer";
...
const Income = () => {
  const expenseamount = useSelector((state) => state.expense.totalexpense);
  ...
  const HandlePdfDownload = () => {
    const data = IncomeReduce.map((ele) => {
      return {
        SPENT: ele.SPENT,
        Des: ele.Des,
  ...
  const IncomeReduce = useSelector((state) => state.income.arr);
  ...
  async function fetchdata() {
    try {
      const res = await axios.get(
        `https://expensetracker-27a82-default-rtdb.firebaseio.com/income/${uid}.json`
      );
      ...
      Dispatch(IncomeReducerAction.setincome2(dataArray));
    } catch (err) {
      console.log("err", IncomeReduce);
    }
  }
  async function DeleteHandler(ele) {
    ...
    try {
      await axios.delete(
        `https://expensetracker-27a82-default-rtdb.firebaseio.com/income/${uid}/${showDelModal.id}.json`
      );
      ...
      // });
      // setexpense(dataArray)
      Dispatch(IncomeReducerAction.setdelete(showDelModal));
  ...
  async function handleexpense(e, e2) {
    ...
    try {
      ...
      Dispatch(IncomeReducerAction.setexpense(obj));
  ...
  async function handleedit(ele) {
    Dispatch(IncomeReducerAction.setdelete(ele));
    ...
    try {
      ...
      Dispatch(IncomeReducerAction.setincome({ ...obj, id: ele.id }));
  ...
  return (
    ...
    >
      ...
      <div
        ...
        } rounded-2xl shadow-xl mb-8 border`}
      >
            ...
            <div className="w-full md:w-auto flex-1">
              ...
              <div className="relative rounded-lg shadow-sm">
                ...
                <input
                  id="spent"
                  type="number"
                  className={`block w-full pl-10 pr-4 py-3 text-lg rounded-lg border ${
                    Theme === "white"
            ...
            <div className="w-full md:w-auto flex-1">
              ...
              <div className="relative">
                <select
                  ...
                  } appearance-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 pr-10`}
                  ref={selectorRef}
                  id="selector"
                >
                  ...
                  <option value="RentIncome">🏡 Rental Income</option>
                  <option value="SideBusiness">🛒 Side Business</option>
      ...
      >
        Total Amount:{" "}
        <span className={Theme === "white" ? "text-teal-700" : "text-teal-300"}>
          {amount}
        </span>
      </p>
      <ProgressBar progress={(expenseamount / amount) * 100} />
      ...
      >
        {/* Table Header */}
        <div
          className={`grid grid-cols-12 p-4 font-medium ${
            Theme === "white"
        ...
        <div
          className={`divide-y ${
            Theme === "white" ? "divide-gray-200" : "divide-gray-700"
          }`}
        >
          {IncomeReduce.map((ele, index) => (
            <div
              key={ele.id || index}
              className={`grid grid-cols-12 items-center p-4 transition-colors duration-150 ${
                Theme === "white" ? "hover:bg-gray-50" : "hover:bg-gray-700/50"
              }`}
            >
              {/* Amount */}
